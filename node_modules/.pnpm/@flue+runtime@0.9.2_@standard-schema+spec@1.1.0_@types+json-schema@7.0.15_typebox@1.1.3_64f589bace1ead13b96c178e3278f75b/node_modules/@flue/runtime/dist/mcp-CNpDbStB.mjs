...
async function connectMcpServerWithClient(name, client, transport) {
	try {
	...
	return tools.map((tool) => {
		...
		return {
			...
			async execute(args, signal) {
				...
				if (tool.execution?.taskSupport === "required") throw new McpError(ErrorCode.InvalidRequest, `Tool "${tool.name}" requires task-based execution. Use client.experimental.tasks.callToolStream() instead.`);
