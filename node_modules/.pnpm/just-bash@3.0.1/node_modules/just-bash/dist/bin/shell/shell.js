...
`,1)}return null}async function q(e,t,s,n){let{stdout:r,stderr:i,exitCode:a}=t,l=t.stdoutKind==="bytes"||t.stdoutKind===void 0&&t.stdoutEncoding==="binary"?"binary":"utf8",c=h=>l;for(let h=0;h<s.length;h++){let y=s[h];if(y.target.type==="HereDoc")continue;let p,w=n?.get(h);if(w!==void 0)p=w;else if(y.operator===">&"||y.operator==="<&"){if(_s(e,y.target)){i+=`bash: $@: ambiguous redirect
...
`,2);let h=t[f];if(!La.includes(h))return _(`complete: ${h}: invalid option name
...
`,2);let c=t[o];if(!Or.includes(c))return _(`compopt: ${c}: invalid option name
...
`,r=!0)}}return k(s,n,r?1:0)}function Ir(e,t){let{filterExport:s,filterReadonly:n,filterNameref:r,filterIndexedArray:i,filterAssocArray:a}=t,o=s||n||r||i||a,l="",c=new Set;for(let f of e.state.env.keys()){if(f.startsWith("BASH_"))continue;if(f.endsWith("__length")){let h=f.slice(0,-8);c.add(h);continue}let d=f.lastIndexOf("_");if(d>0){let h=f.slice(0,d),y=f.slice(d+1);if(/^\d+$/.test(y)||e.state.associativeArrays?.has(h)){c.add(h);continue}}c.add(f)}if(e.state.localVarDepth)for(let f of e.state.localVarDepth.keys())c.add(f);if(e.state.associativeArrays)for(let f of e.state.associativeArrays)c.add(f);let u=Array.from(c).sort();for(let f of u){let d=Dr(e,f),h=e.state.associativeArrays?.has(f),y=ne(e,f),p=!h&&(y.length>0||e.state.env.has(`${f}__length`));if(o&&(a&&!h||i&&!p||s&&!e.state.exportedVars?.has(f)||n&&!e.state.readonlyVars?.has(f)||r&&!ye(e,f)))continue;if(h){let $=Me(e,f);if($.length===0)l+=`declare -A ${f}=()
`;else{let g=$.map(b=>{let m=e.state.env.get(`${f}_${b}`)??"",v=Xs(m);return`['${b}']=${v}`});l+=`declare -A ${f}=(${g.join(" ")})
`}continue}if(y.length>0){let $=y.map(g=>{let b=e.state.env.get(`${f}_${g}`)??"";return`[${g}]=${Ke(b)}`});l+=`declare -a ${f}=(${$.join(" ")})
`;continue}if(e.state.env.has(`${f}__length`)){l+=`declare -a ${f}=()
`;continue}let w=e.state.env.get(f);w!==void 0&&(l+=`declare ${d} ${f}=${Ks(w)}
`)}return M(l)}function xr(e){let t="",s=Array.from(e.state.associativeArrays??[]).sort();for(let n of s){let r=Me(e,n);if(r.length===0)t+=`declare -A ${n}=()
...
`)}return M(t)}function Ys(e,t){e.state.integerVars??=new Set,e.state.integerVars.add(t)}function $t(e,t){return e.state.integerVars?.has(t)??!1}function Qs(e,t){e.state.lowercaseVars??=new Set,e.state.lowercaseVars.add(t),e.state.uppercaseVars?.delete(t)}function Fa(e,t){return e.state.lowercaseVars?.has(t)??!1}function Js(e,t){e.state.uppercaseVars??=new Set,e.state.uppercaseVars.add(t),e.state.lowercaseVars?.delete(t)}function Ma(e,t){return e.state.uppercaseVars?.has(t)??!1}function ft(e,t,s){return Fa(e,t)?s.toLowerCase():Ma(e,t)?s.toUpperCase():s}async function Fr(e,t){try{let s=new V,n=Q(s,t),r=await j(e,n.expression);return String(r)}catch{return"0"}}function Wa(e){let t=e.match(/^[a-zA-Z_][a-zA-Z0-9_]*/);if(!t)return null;let s=t[0],n=s.length;if(e[n]!=="[")return null;let r=0,i=n+1;for(;n<e.length;n++)if(e[n]==="[")r++;else if(e[n]==="]"&&(r--,r===0))break;if(r!==0)return null;let a=e.slice(i,n);if(n++,e[n]!=="=")return null;n++;let o=e.slice(n);return{name:s,indexExpr:a,value:o}}async function en(e,t){let s=!1,n=!1,r=!1,i=!1,a=!1,o=!1,l=!1,c=!1,u=!1,f=!1,d=!1,h=!1,y=!1,p=!1,w=!1,$=[];for(let N=0;N<t.length;N++){let x=t[N];if(x==="-a")s=!0;else if(x==="-A")n=!0;else if(x==="-r")r=!0;else if(x==="-x")i=!0;else if(x==="-p")a=!0;else if(x==="-n")o=!0;else if(x==="+n")l=!0;else if(x==="+a")c=!0;else if(x==="+x")u=!0;else if(x==="--"){$.push(...t.slice(N+1));break}else if(x.startsWith("+")){for(let C of x.slice(1))if(C==="n")l=!0;else if(C==="a")c=!0;else if(C==="x")u=!0;else if(C!=="r"){if(C!=="i"){if(!(C==="f"||C==="F"))return k("",`bash: typeset: +${C}: invalid option
`,2)}}}else if(x==="-i")f=!0;else if(x==="-l")d=!0;else if(x==="-u")h=!0;else if(x==="-f")y=!0;else if(x==="-F")p=!0;else if(x==="-g")w=!0;else if(x.startsWith("-"))for(let C of x.slice(1))if(C==="a")s=!0;else if(C==="A")n=!0;else if(C==="r")r=!0;else if(C==="x")i=!0;else if(C==="p")a=!0;else if(C==="n")o=!0;else if(C==="i")f=!0;else if(C==="l")d=!0;else if(C==="u")h=!0;else if(C==="f")y=!0;else if(C==="F")p=!0;else if(C==="g")w=!0;else return k("",`bash: typeset: -${C}: invalid option
`,2);else $.push(x)}let b=e.state.localScopes.length>0&&!w,m=N=>{if(!b)return;let x=e.state.localScopes[e.state.localScopes.length-1];x.has(N)||x.set(N,e.state.env.get(N))},v=N=>{if(!b)return;let x=e.state.localScopes[e.state.localScopes.length-1];x.has(N)||x.set(N,e.state.env.get(N));let C=`${N}_`;for(let D of e.state.env.keys())D.startsWith(C)&&!D.includes("__")&&(x.has(D)||x.set(D,e.state.env.get(D)));let P=`${N}__length`;e.state.env.has(P)&&!x.has(P)&&x.set(P,e.state.env.get(P))},E=N=>{b&&Ve(e,N)};if(p){if($.length===0){let C=Array.from(e.state.functions.keys()).sort(),P="";for(let D of C)P+=`declare -f ${D}
`;return M(P)}let N=!0,x="";for(let C of $)e.state.functions.has(C)?x+=`${C}
`:N=!1;return k(x,"",N?0:1)}if(y){if($.length===0){let x="",C=Array.from(e.state.functions.keys()).sort();for(let P of C)x+=`${P} ()
{
    # function body
}
`;return M(x)}let N=!0;for(let x of $)e.state.functions.has(x)||(N=!1);return k("","",N?0:1)}if(a&&$.length>0)return Tr(e,$);if(a&&$.length===0)return Ir(e,{filterExport:i,filterReadonly:r,filterNameref:o,filterIndexedArray:s,filterAssocArray:n});if($.length===0&&n&&!a)return xr(e);if($.length===0&&s&&!a)return Rr(e);if($.length===0&&!a)return Lr(e);let S="",O=0;for(let N of $){let x=N.match(/^([a-zA-Z_][a-zA-Z0-9_]*)=\((.*)\)$/s);if(x&&!c){let A=x[1],T=x[2];if(n&&ne(e,A).length>0){S+=`bash: declare: ${A}: cannot convert indexed to associative array
`,O=1;continue}if((s||!n&&!s)&&e.state.associativeArrays?.has(A)){S+=`bash: declare: ${A}: cannot convert associative to indexed array
`,O=1;continue}if(v(A),n&&(e.state.associativeArrays??=new Set,e.state.associativeArrays.add(A)),Ce(e,A),e.state.env.delete(A),e.state.env.delete(`${A}__length`),n&&T.includes("[")){let R=Qt(T);for(let[J,z]of R){let K=G(e,z);e.state.env.set(`${A}_${J}`,K)}}else if(n){let R=Ne(T);for(let J=0;J<R.length;J+=2){let z=R[J],K=J+1<R.length?G(e,R[J+1]):"";e.state.env.set(`${A}_${z}`,K)}}else{let R=Ne(T);if(R.some(z=>/^\[[^\]]+\]=/.test(z))){let z=0;for(let K of R){let re=K.match(/^\[([^\]]+)\]=(.*)$/);if(re){let fe=re[1],Qe=re[2],qi=G(e,Qe),pt;if(/^-?\d+$/.test(fe))pt=Number.parseInt(fe,10);else try{let Gi=new V,Ki=Q(Gi,fe);pt=await j(e,Ki.expression)}catch{pt=0}e.state.env.set(`${A}_${pt}`,qi),z=pt+1}else{let fe=G(e,K);e.state.env.set(`${A}_${z}`,fe),z++}}}else{for(let z=0;z<R.length;z++)e.state.env.set(`${A}_${z}`,R[z]);e.state.env.set(`${A}__length`,String(R.length))}}E(A),r&&ue(e,A),i&&Ae(e,A);continue}if(l){let A=N.includes("=")?N.slice(0,N.indexOf("=")):N;if(Vn(e,A),!N.includes("="))continue}if(u){let A=N.includes("=")?N.slice(0,N.indexOf("=")):N;if(Lt(e,A),!N.includes("="))continue}let C=Wa(N);if(C){let{name:A,indexExpr:T,value:R}=C,J=ee(e,A);if(J)return J;v(A);let z;try{let re=new V,fe=Q(re,T);z=await j(e,fe.expression)}catch{let re=parseInt(T,10);z=Number.isNaN(re)?0:re}e.state.env.set(`${A}_${z}`,R);let K=parseInt(e.state.env.get(`${A}__length`)??"0",10);z>=K&&e.state.env.set(`${A}__length`,String(z+1)),E(A),r&&ue(e,A),i&&Ae(e,A);continue}let P=N.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=\((.*)\)$/s);if(P&&!c){let A=P[1],T=P[2],R=ee(e,A);if(R)return R;v(A);let J=Ne(T);if(e.state.associativeArrays?.has(A)){let z=Qt(T);for(let[K,re]of z){let fe=G(e,re);e.state.env.set(`${A}_${K}`,fe)}}else{let z=ne(e,A),K=0,re=e.state.env.get(A);z.length===0&&re!==void 0?(e.state.env.set(`${A}_0`,re),e.state.env.delete(A),K=1):z.length>0&&(K=Math.max(...z)+1);for(let Qe=0;Qe<J.length;Qe++)e.state.env.set(`${A}_${K+Qe}`,G(e,J[Qe]));let fe=K+J.length;e.state.env.set(`${A}__length`,String(fe))}E(A),r&&ue(e,A),i&&Ae(e,A);continue}let D=N.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=(.*)$/);if(D){let A=D[1],T=G(e,D[2]),R=ee(e,A);if(R)return R;m(A),f&&Ys(e,A),d&&Qs(e,A),h&&Js(e,A);let z=ne(e,A).length>0||e.state.associativeArrays?.has(A);if($t(e,A)){let K=e.state.env.get(A)??"0",re=parseInt(K,10)||0,fe=parseInt(await Fr(e,T),10)||0;T=String(re+fe),e.state.env.set(A,T)}else if(z){T=ft(e,A,T);let K=`${A}_0`,re=e.state.env.get(K)??"";e.state.env.set(K,re+T)}else{T=ft(e,A,T);let K=e.state.env.get(A)??"";e.state.env.set(A,K+T)}E(A),r&&ue(e,A),i&&Ae(e,A),e.state.options.allexport&&!u&&(e.state.exportedVars=e.state.exportedVars||new Set,e.state.exportedVars.add(A));continue}if(N.includes("=")){let A=N.indexOf("="),T=N.slice(0,A),R=N.slice(A+1);if(!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(T)){S+=`bash: typeset: \`${T}': not a valid identifier
...
`,O=1;continue}if(s||n?v(A):m(A),o){Ie(e,A);let R=e.state.env.get(A);R!==void 0&&R!==""&&!/^[a-zA-Z_][a-zA-Z0-9_]*(\[.+\])?$/.test(R)?Bn(e,A):R&&Ss(e,R)&&Es(e,A),E(A),r&&ue(e,A),i&&Ae(e,A);continue}if(f&&Ys(e,A),d&&Qs(e,A),h&&Js(e,A),n){if(ne(e,A).length>0){S+=`bash: declare: ${A}: cannot convert indexed to associative array
...
`}}return M(a)}for(let a of i){let o=a.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=\((.*)\)$/s);if(o){let f=o[1],d=o[2],h=ee(e,f);if(h)return h;let y=Ne(d);if(e.state.associativeArrays?.has(f)){let p=Qt(d);for(let[w,$]of p){let g=G(e,$);e.state.env.set(`${f}_${w}`,g)}}else{let p=ne(e,f),w=0,$=e.state.env.get(f);p.length===0&&$!==void 0?(e.state.env.set(`${f}_0`,$),e.state.env.delete(f),w=1):p.length>0&&(w=Math.max(...p)+1);for(let b=0;b<y.length;b++)e.state.env.set(`${f}_${w+b}`,G(e,y[b]));let g=w+y.length;e.state.env.set(`${f}__length`,String(g))}ue(e,f);continue}let l=a.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=(.*)$/);if(l){let f=l[1],d=G(e,l[2]),h=ee(e,f);if(h)return h;let y=e.state.env.get(f)??"";e.state.env.set(f,y+d),ue(e,f);continue}let c=$r(a);if(c.value===void 0&&!c.isArray){ue(e,c.name);continue}let u=await Er(e,c,{makeReadonly:!0});if(u)return u}return F}function sn(e){return e.state.directoryStack??=[],e.state.directoryStack}function Et(e,t){return t&&e===t?"~":t&&e.startsWith(`${t}/`)?`~${e.slice(t.length)}`:e}function za(e){let t=e.split("/").filter(n=>n&&n!=="."),s=[];for(let n of t)n===".."?s.pop():s.push(n);return`/${s.join("/")}`}async function nn(e,t){let s=sn(e),n;for(let o=0;o<t.length;o++){let l=t[o];if(l==="--"){if(o+1<t.length){if(n!==void 0)return _(`bash: pushd: too many arguments
...
`,1);let o=s[0];s[0]=s[1],s[1]=o,n=s[0]}let r;if(n.startsWith("/"))r=n;else if(n===".."){let o=e.state.cwd.split("/").filter(l=>l);o.pop(),r=`/${o.join("/")}`}else n==="."?r=e.state.cwd:n.startsWith("~")?r=(e.state.env.get("HOME")||"/")+n.slice(1):r=`${e.state.cwd}/${n}`;r=za(r);try{if(!(await e.fs.stat(r)).isDirectory)return _(`bash: pushd: ${n}: Not a directory
...
`),c+=`
`):i?c=o.map(u=>r?u:Et(u,l)).join(`
`)+`
`:c=o.map(u=>r?u:Et(u,l)).join(" ")+`
...
`,1);let y=!1,p="";for(let w of l)e.state.hashTable.has(w)?e.state.hashTable.delete(w):(p+=`bash: hash: ${w}: not found
`,y=!0);return y?_(p,1):F}if(a){if(l.length===0)return _(`bash: hash: -t: option requires an argument
`,1);let y="",p=!1,w="";for(let $ of l){let g=e.state.hashTable.get($);g?l.length>1?y+=`${$}	${g}
`:y+=`${g}
`:(w+=`bash: hash: ${$}: not found
`,p=!0)}return p?{exitCode:1,stdout:y,stderr:w}:M(y)}if(i){if(l.length===0)return _(`bash: hash: usage: hash [-lr] [-p pathname] [-dt] [name ...]
`,1);let y=l[0];return e.state.hashTable.set(y,o),F}if(l.length===0){if(e.state.hashTable.size===0)return M(`hash: hash table empty
`);let y="";if(r)for(let[p,w]of e.state.hashTable)y+=`builtin hash -p ${w} ${p}
`;else{y=`hits	command
`;for(let[,p]of e.state.hashTable)y+=`   1	${p}
`}return M(y)}let u=!1,f="",h=(e.state.env.get("PATH")||"/usr/bin:/bin").split(":");for(let y of l){if(y.includes("/")){f+=`bash: hash: ${y}: cannot use / in name
`,u=!0;continue}let p=!1;for(let w of h){if(!w)continue;let $=`${w}/${y}`;if(await e.fs.exists($)){e.state.hashTable.set(y,$),p=!0;break}}p||(f+=`bash: hash: ${y}: not found
`,u=!0)}return u?_(f,1):F}var Mr=new Map([[":",[": [arguments]",`Null command.
    ...
    option is given.`]]]),Wr=[...Mr.keys()].sort();function un(e,t){let s=!1,n=[],r=0;for(;r<t.length;){let l=t[r];if(l==="--"){for(r++;r<t.length;)n.push(t[r]),r++;break}if(l.startsWith("-")&&l.length>1){for(let c=1;c<l.length;c++){let u=l[c];if(u==="s")s=!0;else return _(`bash: help: -${u}: invalid option
`,2)}r++}else n.push(l),r++}if(n.length===0)return Ba();let i="",a=!1,o="";for(let l of n){let c=Va(l);if(c.length===0){o+=`bash: help: no help topics match \`${l}'.  Try \`help help' or \`man -k ${l}' or \`info ${l}'.
`,a=!0;continue}for(let u of c){let f=Mr.get(u);if(!f)continue;let[d,h]=f;s?i+=`${u}: ${d}
`:i+=`${u}: ${d}
${h}
`}}return{exitCode:a?1:0,stdout:i,stderr:o}}function Va(e){let t=e.replace(/[.+^${}()|[\]\\]/g,"\\$&").replace(/\*/g,".*").replace(/\?/g,"."),s=mt(`^${t}$`);return Wr.filter(n=>s.test(n))}function Ba(){let e=[];e.push("just-bash shell builtins"),e.push("These shell commands are defined internally. Type `help' to see this list."),e.push("Type `help name' to find out more about the function `name'."),e.push("");let t=36,s=Wr.slice(),n=Math.ceil(s.length/2);for(let r=0;r<n;r++){let i=s[r]||"",a=s[r+n]||"",o=i.padEnd(t);e.push(a?`${o}${a}`:i)}return M(`${e.join(`
`)}
`)}function ja(e){let t=[],s="",n=0;for(let r of e){for(let i of r)i==="("?n++:i===")"&&n--;s?s+=` ${r}`:s=r,n===0&&(t.push(s),s="")}return s&&t.push(s),t}async function fn(e,t){if(t.length===0)return _(`bash: let: expression expected
`);let s=ja(t),n=0;for(let r of s)try{let a=Ee(`(( ${r} ))`).statements[0];if(a&&a.pipelines.length>0&&a.pipelines[0].commands.length>0){let o=a.pipelines[0].commands[0];o.type==="ArithmeticCommand"&&(n=await j(e,o.expression.expression))}}catch(i){return _(`bash: let: ${r}: ${i.message}
...
`)}return k(c,"",0)}for(let c of l){let u,f,d=c.match(/^([a-zA-Z_][a-zA-Z0-9_]*)=\((.*)\)$/s);if(d){u=d[1];let $=d[2];if(!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(u)){n+=`bash: local: \`${c}': not a valid identifier
`,r=1;continue}if(ee(e,u,"bash"),!s.has(u)){s.set(u,e.state.env.get(u));let m=`${u}_`;for(let v of e.state.env.keys())v.startsWith(m)&&!v.includes("__")&&(s.has(v)||s.set(v,e.state.env.get(v)))}let g=`${u}_`;for(let m of e.state.env.keys())m.startsWith(g)&&!m.includes("__")&&e.state.env.delete(m);let b=Ne($);for(let m=0;m<b.length;m++)e.state.env.set(`${u}_${m}`,b[m]);e.state.env.set(`${u}__length`,String(b.length)),Ve(e,u),i&&Ie(e,u);continue}let h=c.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=\((.*)\)$/s);if(h){u=h[1];let $=h[2];if(ee(e,u,"bash"),!s.has(u)){s.set(u,e.state.env.get(u));let S=`${u}_`;for(let N of e.state.env.keys())N.startsWith(S)&&!N.includes("__")&&(s.has(N)||s.set(N,e.state.env.get(N)));let O=`${u}__length`;e.state.env.has(O)&&!s.has(O)&&s.set(O,e.state.env.get(O))}let g=Ne($),b=ne(e,u),m=0,v=e.state.env.get(u);b.length===0&&v!==void 0?(e.state.env.set(`${u}_0`,v),e.state.env.delete(u),m=1):b.length>0&&(m=Math.max(...b)+1);for(let S=0;S<g.length;S++)e.state.env.set(`${u}_${m+S}`,G(e,g[S]));let E=m+g.length;e.state.env.set(`${u}__length`,String(E)),Ve(e,u),i&&Ie(e,u);continue}let y=c.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=(.*)$/);if(y){u=y[1];let $=G(e,y[2]);ee(e,u,"bash"),s.has(u)||s.set(u,e.state.env.get(u));let g=e.state.env.get(u)??"";e.state.env.set(u,g+$),Ve(e,u),i&&Ie(e,u);continue}let p=c.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\[([^\]]+)\]=(.*)$/s);if(p){u=p[1];let $=p[2],g=G(e,p[3]);if(ee(e,u,"bash"),!s.has(u)){s.set(u,e.state.env.get(u));let v=`${u}_`;for(let S of e.state.env.keys())S.startsWith(v)&&!S.includes("__")&&(s.has(S)||s.set(S,e.state.env.get(S)));let E=`${u}__length`;e.state.env.has(E)&&!s.has(E)&&s.set(E,e.state.env.get(E))}let b;try{let v=new V,E=Q(v,$);b=await j(e,E.expression)}catch{let v=parseInt($,10);b=Number.isNaN(v)?0:v}e.state.env.set(`${u}_${b}`,g);let m=parseInt(e.state.env.get(`${u}__length`)??"0",10);b>=m&&e.state.env.set(`${u}__length`,String(b+1)),Ve(e,u),i&&Ie(e,u);continue}if(c.includes("=")){let $=c.indexOf("=");u=c.slice(0,$),f=G(e,c.slice($+1))}else u=c;if(!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(u)){n+=`bash: local: \`${c}': not a valid identifier
`,r=1;continue}let w=s.has(u);if(f!==void 0){let $=e.state.env.get(u);if(e.state.tempEnvBindings){let g=e.state.accessedTempEnvVars?.has(u),b=e.state.mutatedTempEnvVars?.has(u);if(!g&&!b)for(let m=e.state.tempEnvBindings.length-1;m>=0;m--){let v=e.state.tempEnvBindings[m];if(v.has(u)){$=v.get(u);break}}}Sr(e,u,$)}if(!w){let $=e.state.env.get(u);if(e.state.tempEnvBindings)for(let g=e.state.tempEnvBindings.length-1;g>=0;g--){let b=e.state.tempEnvBindings[g];if(b.has(u)){$=b.get(u);break}}if(s.set(u,$),a){let g=`${u}_`;for(let m of e.state.env.keys())m.startsWith(g)&&!m.includes("__")&&(s.has(m)||s.set(m,e.state.env.get(m)));let b=`${u}__length`;e.state.env.has(b)&&!s.has(b)&&s.set(b,e.state.env.get(b))}}if(a&&f===void 0){let $=`${u}_`;for(let g of e.state.env.keys())g.startsWith($)&&!g.includes("__")&&e.state.env.delete(g);e.state.env.set(`${u}__length`,"0")}else if(f!==void 0){if(ee(e,u,"bash"),i&&f!==""&&!/^[a-zA-Z_][a-zA-Z0-9_]*(\[.+\])?$/.test(f)){n+=`bash: local: \`${f}': invalid variable name for name reference
`,r=1;continue}e.state.env.set(u,f),e.state.options.allexport&&(e.state.exportedVars=e.state.exportedVars||new Set,e.state.exportedVars.add(u))}else{let $=e.state.tempEnvBindings?.some(g=>g.has(u));!w&&!$&&e.state.env.delete(u)}Ve(e,u),i&&Ie(e,u)}return k("",n,r)}function hn(e,t,s){let n=`
...
`)}
`,stderr:""}}let c=!1,u="",f=[];for(let d of o){if(!os(d)&&!eo(d)){u+=`shopt: ${d}: invalid shell option name
...
`);let[,...a]=r;return n(i,a,[],s,!0,!1,-1)}async function ii(e,t,s,n,r){let{ctx:i,buildExportedEnv:a,executeUserScript:o}=e,c=await Xr(i,t,r?"/usr/bin:/bin":void 0);if(!c)return dr(t)?_(`bash: ${t}: command not available in browser environments. Exclude '${t}' from your commands or use the Node.js bundle.
...
`,1)}}}return{index:r}}async function go(e,t,s,n,r,i){let a="",o=s,l=null;if(ye(e,s)){let f=Hn(e,s,n);if(f===void 0)return{continueToNext:!1,xtraceOutput:"",error:k("",`bash: ${s}: circular name reference
`,1)};if(f===null)return{continueToNext:!0,xtraceOutput:""};o=f;let d=o.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\[(.+)\]$/);d&&(l={arrayName:d[1],subscriptExpr:d[2]},o=d[1])}if(Ze(e,o)){if(t.name)return a+=`bash: ${o}: readonly variable
...
`,1);return q(e,O,t.redirections)}v();let E=k(g,b,m);return q(e,E,t.redirections)}async function Ei(e,t,s,n){let r="",i="",a=0,o=await ts(e,t.redirections);if(o)return o;let l=s;for(let f of t.redirections)if((f.operator==="<<"||f.operator==="<<-")&&f.target.type==="HereDoc"){let d=f.target,h=await I(e,d.content);d.stripTabs&&(h=h.split(`
...
`)}prompt(){this.rl.question(this.getPrompt(),async t=>{this.running&&(await this.executeCommand(t),this.prompt())})}async run(){if(this.isInteractive)this.printWelcome(),this.prompt();else{let t=[];this.rl.on("line",s=>{t.push(s)}),await new Promise(s=>{this.rl.on("close",s)});for(let s of t)await this.executeCommand(s)}}};function cl(){let e=process.argv.slice(2),t={};for(let s=0;s<e.length;s++)if(e[s]==="--cwd"&&e[s+1])t.cwd=e[++s];else if(e[s]==="--files"&&e[s+1]){let n=e[++s];try{let r=Ui.readFileSync(n,"utf-8");t.files=JSON.parse(r)}catch(r){console.error(`Error reading files from ${n}:`,r),process.exit(1)}}else e[s]==="--network"?t.network=!0:e[s]==="--no-network"?t.network=!1:(e[s]==="--help"||e[s]==="-h")&&(console.log(`
...
Example:
  pnpm shell
  pnpm shell --network
`),process.exit(0));return t}var ul=cl(),fl=new Nn(ul);fl.run();
