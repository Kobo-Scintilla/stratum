...
`,1)}return null}async function q(e,t,s,n){let{stdout:r,stderr:i,exitCode:a}=t,l=t.stdoutKind==="bytes"||t.stdoutKind===void 0&&t.stdoutEncoding==="binary"?"binary":"utf8",c=h=>l;for(let h=0;h<s.length;h++){let y=s[h];if(y.target.type==="HereDoc")continue;let p,w=n?.get(h);if(w!==void 0)p=w;else if(y.operator===">&"||y.operator==="<&"){if(As(e,y.target)){i+=`bash: $@: ambiguous redirect
...
`,2);let h=t[f];if(!Ia.includes(h))return _(`complete: ${h}: invalid option name
...
`,2);let c=t[o];if(!kr.includes(c))return _(`compopt: ${c}: invalid option name
...
`)}return M(t)}function Xs(e,t){e.state.integerVars??=new Set,e.state.integerVars.add(t)}function $t(e,t){return e.state.integerVars?.has(t)??!1}function Ys(e,t){e.state.lowercaseVars??=new Set,e.state.lowercaseVars.add(t),e.state.uppercaseVars?.delete(t)}function Ra(e,t){return e.state.lowercaseVars?.has(t)??!1}function Qs(e,t){e.state.uppercaseVars??=new Set,e.state.uppercaseVars.add(t),e.state.lowercaseVars?.delete(t)}function La(e,t){return e.state.uppercaseVars?.has(t)??!1}function ut(e,t,s){return Ra(e,t)?s.toLowerCase():La(e,t)?s.toUpperCase():s}async function Rr(e,t){try{let s=new V,n=Q(s,t),r=await j(e,n.expression);return String(r)}catch{return"0"}}function Fa(e){let t=e.match(/^[a-zA-Z_][a-zA-Z0-9_]*/);if(!t)return null;let s=t[0],n=s.length;if(e[n]!=="[")return null;let r=0,i=n+1;for(;n<e.length;n++)if(e[n]==="[")r++;else if(e[n]==="]"&&(r--,r===0))break;if(r!==0)return null;let a=e.slice(i,n);if(n++,e[n]!=="=")return null;n++;let o=e.slice(n);return{name:s,indexExpr:a,value:o}}async function Js(e,t){let s=!1,n=!1,r=!1,i=!1,a=!1,o=!1,l=!1,c=!1,u=!1,f=!1,d=!1,h=!1,y=!1,p=!1,w=!1,$=[];for(let N=0;N<t.length;N++){let I=t[N];if(I==="-a")s=!0;else if(I==="-A")n=!0;else if(I==="-r")r=!0;else if(I==="-x")i=!0;else if(I==="-p")a=!0;else if(I==="-n")o=!0;else if(I==="+n")l=!0;else if(I==="+a")c=!0;else if(I==="+x")u=!0;else if(I==="--"){$.push(...t.slice(N+1));break}else if(I.startsWith("+")){for(let C of I.slice(1))if(C==="n")l=!0;else if(C==="a")c=!0;else if(C==="x")u=!0;else if(C!=="r"){if(C!=="i"){if(!(C==="f"||C==="F"))return P("",`bash: typeset: +${C}: invalid option
`,2)}}}else if(I==="-i")f=!0;else if(I==="-l")d=!0;else if(I==="-u")h=!0;else if(I==="-f")y=!0;else if(I==="-F")p=!0;else if(I==="-g")w=!0;else if(I.startsWith("-"))for(let C of I.slice(1))if(C==="a")s=!0;else if(C==="A")n=!0;else if(C==="r")r=!0;else if(C==="x")i=!0;else if(C==="p")a=!0;else if(C==="n")o=!0;else if(C==="i")f=!0;else if(C==="l")d=!0;else if(C==="u")h=!0;else if(C==="f")y=!0;else if(C==="F")p=!0;else if(C==="g")w=!0;else return P("",`bash: typeset: -${C}: invalid option
`,2);else $.push(I)}let b=e.state.localScopes.length>0&&!w,m=N=>{if(!b)return;let I=e.state.localScopes[e.state.localScopes.length-1];I.has(N)||I.set(N,e.state.env.get(N))},v=N=>{if(!b)return;let I=e.state.localScopes[e.state.localScopes.length-1];I.has(N)||I.set(N,e.state.env.get(N));let C=`${N}_`;for(let D of e.state.env.keys())D.startsWith(C)&&!D.includes("__")&&(I.has(D)||I.set(D,e.state.env.get(D)));let k=`${N}__length`;e.state.env.has(k)&&!I.has(k)&&I.set(k,e.state.env.get(k))},E=N=>{b&&ze(e,N)};if(p){if($.length===0){let C=Array.from(e.state.functions.keys()).sort(),k="";for(let D of C)k+=`declare -f ${D}
`;return M(k)}let N=!0,I="";for(let C of $)e.state.functions.has(C)?I+=`${C}
`:N=!1;return P(I,"",N?0:1)}if(y){if($.length===0){let I="",C=Array.from(e.state.functions.keys()).sort();for(let k of C)I+=`${k} ()
{
    # function body
}
`;return M(I)}let N=!0;for(let I of $)e.state.functions.has(I)||(N=!1);return P("","",N?0:1)}if(a&&$.length>0)return Or(e,$);if(a&&$.length===0)return Dr(e,{filterExport:i,filterReadonly:r,filterNameref:o,filterIndexedArray:s,filterAssocArray:n});if($.length===0&&n&&!a)return Tr(e);if($.length===0&&s&&!a)return xr(e);if($.length===0&&!a)return Ir(e);let S="",O=0;for(let N of $){let I=N.match(/^([a-zA-Z_][a-zA-Z0-9_]*)=\((.*)\)$/s);if(I&&!c){let A=I[1],T=I[2];if(n&&se(e,A).length>0){S+=`bash: declare: ${A}: cannot convert indexed to associative array
`,O=1;continue}if((s||!n&&!s)&&e.state.associativeArrays?.has(A)){S+=`bash: declare: ${A}: cannot convert associative to indexed array
`,O=1;continue}if(v(A),n&&(e.state.associativeArrays??=new Set,e.state.associativeArrays.add(A)),_e(e,A),e.state.env.delete(A),e.state.env.delete(`${A}__length`),n&&T.includes("[")){let R=Qt(T);for(let[J,z]of R){let K=G(e,z);e.state.env.set(`${A}_${J}`,K)}}else if(n){let R=ke(T);for(let J=0;J<R.length;J+=2){let z=R[J],K=J+1<R.length?G(e,R[J+1]):"";e.state.env.set(`${A}_${z}`,K)}}else{let R=ke(T);if(R.some(z=>/^\[[^\]]+\]=/.test(z))){let z=0;for(let K of R){let ne=K.match(/^\[([^\]]+)\]=(.*)$/);if(ne){let ue=ne[1],Ye=ne[2],Ui=G(e,Ye),pt;if(/^-?\d+$/.test(ue))pt=Number.parseInt(ue,10);else try{let Zi=new V,qi=Q(Zi,ue);pt=await j(e,qi.expression)}catch{pt=0}e.state.env.set(`${A}_${pt}`,Ui),z=pt+1}else{let ue=G(e,K);e.state.env.set(`${A}_${z}`,ue),z++}}}else{for(let z=0;z<R.length;z++)e.state.env.set(`${A}_${z}`,R[z]);e.state.env.set(`${A}__length`,String(R.length))}}E(A),r&&ce(e,A),i&&Se(e,A);continue}if(l){let A=N.includes("=")?N.slice(0,N.indexOf("=")):N;if(Wn(e,A),!N.includes("="))continue}if(u){let A=N.includes("=")?N.slice(0,N.indexOf("=")):N;if(Lt(e,A),!N.includes("="))continue}let C=Fa(N);if(C){let{name:A,indexExpr:T,value:R}=C,J=ee(e,A);if(J)return J;v(A);let z;try{let ne=new V,ue=Q(ne,T);z=await j(e,ue.expression)}catch{let ne=parseInt(T,10);z=Number.isNaN(ne)?0:ne}e.state.env.set(`${A}_${z}`,R);let K=parseInt(e.state.env.get(`${A}__length`)??"0",10);z>=K&&e.state.env.set(`${A}__length`,String(z+1)),E(A),r&&ce(e,A),i&&Se(e,A);continue}let k=N.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=\((.*)\)$/s);if(k&&!c){let A=k[1],T=k[2],R=ee(e,A);if(R)return R;v(A);let J=ke(T);if(e.state.associativeArrays?.has(A)){let z=Qt(T);for(let[K,ne]of z){let ue=G(e,ne);e.state.env.set(`${A}_${K}`,ue)}}else{let z=se(e,A),K=0,ne=e.state.env.get(A);z.length===0&&ne!==void 0?(e.state.env.set(`${A}_0`,ne),e.state.env.delete(A),K=1):z.length>0&&(K=Math.max(...z)+1);for(let Ye=0;Ye<J.length;Ye++)e.state.env.set(`${A}_${K+Ye}`,G(e,J[Ye]));let ue=K+J.length;e.state.env.set(`${A}__length`,String(ue))}E(A),r&&ce(e,A),i&&Se(e,A);continue}let D=N.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=(.*)$/);if(D){let A=D[1],T=G(e,D[2]),R=ee(e,A);if(R)return R;m(A),f&&Xs(e,A),d&&Ys(e,A),h&&Qs(e,A);let z=se(e,A).length>0||e.state.associativeArrays?.has(A);if($t(e,A)){let K=e.state.env.get(A)??"0",ne=parseInt(K,10)||0,ue=parseInt(await Rr(e,T),10)||0;T=String(ne+ue),e.state.env.set(A,T)}else if(z){T=ut(e,A,T);let K=`${A}_0`,ne=e.state.env.get(K)??"";e.state.env.set(K,ne+T)}else{T=ut(e,A,T);let K=e.state.env.get(A)??"";e.state.env.set(A,K+T)}E(A),r&&ce(e,A),i&&Se(e,A),e.state.options.allexport&&!u&&(e.state.exportedVars=e.state.exportedVars||new Set,e.state.exportedVars.add(A));continue}if(N.includes("=")){let A=N.indexOf("="),T=N.slice(0,A),R=N.slice(A+1);if(!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(T)){S+=`bash: typeset: \`${T}': not a valid identifier
...
`,O=1;continue}if(s||n?v(A):m(A),o){Te(e,A);let R=e.state.env.get(A);R!==void 0&&R!==""&&!/^[a-zA-Z_][a-zA-Z0-9_]*(\[.+\])?$/.test(R)?zn(e,A):R&&Es(e,R)&&$s(e,A),E(A),r&&ce(e,A),i&&Se(e,A);continue}if(f&&Xs(e,A),d&&Ys(e,A),h&&Qs(e,A),n){if(se(e,A).length>0){S+=`bash: declare: ${A}: cannot convert indexed to associative array
...
`}}return M(a)}for(let a of i){let o=a.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=\((.*)\)$/s);if(o){let f=o[1],d=o[2],h=ee(e,f);if(h)return h;let y=ke(d);if(e.state.associativeArrays?.has(f)){let p=Qt(d);for(let[w,$]of p){let g=G(e,$);e.state.env.set(`${f}_${w}`,g)}}else{let p=se(e,f),w=0,$=e.state.env.get(f);p.length===0&&$!==void 0?(e.state.env.set(`${f}_0`,$),e.state.env.delete(f),w=1):p.length>0&&(w=Math.max(...p)+1);for(let b=0;b<y.length;b++)e.state.env.set(`${f}_${w+b}`,G(e,y[b]));let g=w+y.length;e.state.env.set(`${f}__length`,String(g))}ce(e,f);continue}let l=a.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=(.*)$/);if(l){let f=l[1],d=G(e,l[2]),h=ee(e,f);if(h)return h;let y=e.state.env.get(f)??"";e.state.env.set(f,y+d),ce(e,f);continue}let c=vr(a);if(c.value===void 0&&!c.isArray){ce(e,c.name);continue}let u=await br(e,c,{makeReadonly:!0});if(u)return u}return F}function tn(e){return e.state.directoryStack??=[],e.state.directoryStack}function Et(e,t){return t&&e===t?"~":t&&e.startsWith(`${t}/`)?`~${e.slice(t.length)}`:e}function Ma(e){let t=e.split("/").filter(n=>n&&n!=="."),s=[];for(let n of t)n===".."?s.pop():s.push(n);return`/${s.join("/")}`}async function sn(e,t){let s=tn(e),n;for(let o=0;o<t.length;o++){let l=t[o];if(l==="--"){if(o+1<t.length){if(n!==void 0)return _(`bash: pushd: too many arguments
...
`,1);let o=s[0];s[0]=s[1],s[1]=o,n=s[0]}let r;if(n.startsWith("/"))r=n;else if(n===".."){let o=e.state.cwd.split("/").filter(l=>l);o.pop(),r=`/${o.join("/")}`}else n==="."?r=e.state.cwd:n.startsWith("~")?r=(e.state.env.get("HOME")||"/")+n.slice(1):r=`${e.state.cwd}/${n}`;r=Ma(r);try{if(!(await e.fs.stat(r)).isDirectory)return _(`bash: pushd: ${n}: Not a directory
...
`,u=!0)}return u?_(f,1):F}var Lr=new Map([[":",[": [arguments]",`Null command.
    ...
    option is given.`]]]),Fr=[...Lr.keys()].sort();function cn(e,t){let s=!1,n=[],r=0;for(;r<t.length;){let l=t[r];if(l==="--"){for(r++;r<t.length;)n.push(t[r]),r++;break}if(l.startsWith("-")&&l.length>1){for(let c=1;c<l.length;c++){let u=l[c];if(u==="s")s=!0;else return _(`bash: help: -${u}: invalid option
`,2)}r++}else n.push(l),r++}if(n.length===0)return za();let i="",a=!1,o="";for(let l of n){let c=Wa(l);if(c.length===0){o+=`bash: help: no help topics match \`${l}'.  Try \`help help' or \`man -k ${l}' or \`info ${l}'.
`,a=!0;continue}for(let u of c){let f=Lr.get(u);if(!f)continue;let[d,h]=f;s?i+=`${u}: ${d}
`:i+=`${u}: ${d}
${h}
`}}return{exitCode:a?1:0,stdout:i,stderr:o}}function Wa(e){let t=e.replace(/[.+^${}()|[\]\\]/g,"\\$&").replace(/\*/g,".*").replace(/\?/g,"."),s=mt(`^${t}$`);return Fr.filter(n=>s.test(n))}function za(){let e=[];e.push("just-bash shell builtins"),e.push("These shell commands are defined internally. Type `help' to see this list."),e.push("Type `help name' to find out more about the function `name'."),e.push("");let t=36,s=Fr.slice(),n=Math.ceil(s.length/2);for(let r=0;r<n;r++){let i=s[r]||"",a=s[r+n]||"",o=i.padEnd(t);e.push(a?`${o}${a}`:i)}return M(`${e.join(`
`)}
`)}function Va(e){let t=[],s="",n=0;for(let r of e){for(let i of r)i==="("?n++:i===")"&&n--;s?s+=` ${r}`:s=r,n===0&&(t.push(s),s="")}return s&&t.push(s),t}async function un(e,t){if(t.length===0)return _(`bash: let: expression expected
`);let s=Va(t),n=0;for(let r of s)try{let a=$e(`(( ${r} ))`).statements[0];if(a&&a.pipelines.length>0&&a.pipelines[0].commands.length>0){let o=a.pipelines[0].commands[0];o.type==="ArithmeticCommand"&&(n=await j(e,o.expression.expression))}}catch(i){return _(`bash: let: ${r}: ${i.message}
...
`)}return P(c,"",0)}for(let c of l){let u,f,d=c.match(/^([a-zA-Z_][a-zA-Z0-9_]*)=\((.*)\)$/s);if(d){u=d[1];let $=d[2];if(!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(u)){n+=`bash: local: \`${c}': not a valid identifier
`,r=1;continue}if(ee(e,u,"bash"),!s.has(u)){s.set(u,e.state.env.get(u));let m=`${u}_`;for(let v of e.state.env.keys())v.startsWith(m)&&!v.includes("__")&&(s.has(v)||s.set(v,e.state.env.get(v)))}let g=`${u}_`;for(let m of e.state.env.keys())m.startsWith(g)&&!m.includes("__")&&e.state.env.delete(m);let b=ke($);for(let m=0;m<b.length;m++)e.state.env.set(`${u}_${m}`,b[m]);e.state.env.set(`${u}__length`,String(b.length)),ze(e,u),i&&Te(e,u);continue}let h=c.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=\((.*)\)$/s);if(h){u=h[1];let $=h[2];if(ee(e,u,"bash"),!s.has(u)){s.set(u,e.state.env.get(u));let S=`${u}_`;for(let N of e.state.env.keys())N.startsWith(S)&&!N.includes("__")&&(s.has(N)||s.set(N,e.state.env.get(N)));let O=`${u}__length`;e.state.env.has(O)&&!s.has(O)&&s.set(O,e.state.env.get(O))}let g=ke($),b=se(e,u),m=0,v=e.state.env.get(u);b.length===0&&v!==void 0?(e.state.env.set(`${u}_0`,v),e.state.env.delete(u),m=1):b.length>0&&(m=Math.max(...b)+1);for(let S=0;S<g.length;S++)e.state.env.set(`${u}_${m+S}`,G(e,g[S]));let E=m+g.length;e.state.env.set(`${u}__length`,String(E)),ze(e,u),i&&Te(e,u);continue}let y=c.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\+=(.*)$/);if(y){u=y[1];let $=G(e,y[2]);ee(e,u,"bash"),s.has(u)||s.set(u,e.state.env.get(u));let g=e.state.env.get(u)??"";e.state.env.set(u,g+$),ze(e,u),i&&Te(e,u);continue}let p=c.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\[([^\]]+)\]=(.*)$/s);if(p){u=p[1];let $=p[2],g=G(e,p[3]);if(ee(e,u,"bash"),!s.has(u)){s.set(u,e.state.env.get(u));let v=`${u}_`;for(let S of e.state.env.keys())S.startsWith(v)&&!S.includes("__")&&(s.has(S)||s.set(S,e.state.env.get(S)));let E=`${u}__length`;e.state.env.has(E)&&!s.has(E)&&s.set(E,e.state.env.get(E))}let b;try{let v=new V,E=Q(v,$);b=await j(e,E.expression)}catch{let v=parseInt($,10);b=Number.isNaN(v)?0:v}e.state.env.set(`${u}_${b}`,g);let m=parseInt(e.state.env.get(`${u}__length`)??"0",10);b>=m&&e.state.env.set(`${u}__length`,String(b+1)),ze(e,u),i&&Te(e,u);continue}if(c.includes("=")){let $=c.indexOf("=");u=c.slice(0,$),f=G(e,c.slice($+1))}else u=c;if(!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(u)){n+=`bash: local: \`${c}': not a valid identifier
`,r=1;continue}let w=s.has(u);if(f!==void 0){let $=e.state.env.get(u);if(e.state.tempEnvBindings){let g=e.state.accessedTempEnvVars?.has(u),b=e.state.mutatedTempEnvVars?.has(u);if(!g&&!b)for(let m=e.state.tempEnvBindings.length-1;m>=0;m--){let v=e.state.tempEnvBindings[m];if(v.has(u)){$=v.get(u);break}}}$r(e,u,$)}if(!w){let $=e.state.env.get(u);if(e.state.tempEnvBindings)for(let g=e.state.tempEnvBindings.length-1;g>=0;g--){let b=e.state.tempEnvBindings[g];if(b.has(u)){$=b.get(u);break}}if(s.set(u,$),a){let g=`${u}_`;for(let m of e.state.env.keys())m.startsWith(g)&&!m.includes("__")&&(s.has(m)||s.set(m,e.state.env.get(m)));let b=`${u}__length`;e.state.env.has(b)&&!s.has(b)&&s.set(b,e.state.env.get(b))}}if(a&&f===void 0){let $=`${u}_`;for(let g of e.state.env.keys())g.startsWith($)&&!g.includes("__")&&e.state.env.delete(g);e.state.env.set(`${u}__length`,"0")}else if(f!==void 0){if(ee(e,u,"bash"),i&&f!==""&&!/^[a-zA-Z_][a-zA-Z0-9_]*(\[.+\])?$/.test(f)){n+=`bash: local: \`${f}': invalid variable name for name reference
`,r=1;continue}e.state.env.set(u,f),e.state.options.allexport&&(e.state.exportedVars=e.state.exportedVars||new Set,e.state.exportedVars.add(u))}else{let $=e.state.tempEnvBindings?.some(g=>g.has(u));!w&&!$&&e.state.env.delete(u)}ze(e,u),i&&Te(e,u)}return P("",n,r)}function dn(e,t,s){let n=`
...
`)}
`,stderr:""}}let c=!1,u="",f=[];for(let d of o){if(!os(d)&&!Qa(d)){u+=`shopt: ${d}: invalid shell option name
...
`;else{let E=e.state.functions.get(p),S=E?Xr(p,E):`${p} is a function
`;u+=S}w=!0}let g=e.state.env.get(`BASH_ALIAS_${p}`);if(g!==void 0&&(o||!w)&&(i||(r?u+=`alias
`:u+=`${p} is aliased to \`${g}'
`),w=!0,!o)||Ms.has(p)&&(o||!w)&&(i||(r?u+=`keyword
`:u+=`${p} is a shell keyword
`),w=!0,!o))continue;if(!o&&$&&!w){if(!i)if(r)u+=`function
`;else{let E=e.state.functions.get(p),S=E?Xr(p,E):`${p} is a function
`;u+=S}w=!0;continue}if(!(lt.has(p)&&(o||!w)&&(i||(r?u+=`builtin
...
`)}}}return i?d=y&&!h?1:0:d=y?1:0,P(u,f,d)}function Xr(e,t){let s;return t.body.type==="Group"?s=t.body.body.map(r=>St(r)).join("; "):s=St(t.body),`${e} is a function
${e} () 
{ 
    ${s}
}
`}function St(e){if(Array.isArray(e))return e.map(t=>St(t)).join("; ");if(e.type==="Statement"){let t=[];for(let s=0;s<e.pipelines.length;s++){let n=e.pipelines[s];t.push(so(n)),e.operators[s]&&t.push(e.operators[s])}return t.join(" ")}if(e.type==="SimpleCommand"){let t=e,s=[];t.name&&s.push(Yr(t.name));for(let n of t.args)s.push(Yr(n));return s.join(" ")}return e.type==="Group"?`{ ${e.body.map(n=>St(n)).join("; ")}; }`:"..."}function so(e){let t=e.commands.map(s=>St(s));return(e.negated?"! ":"")+t.join(" | ")}function Yr(e){let t="";for(let s of e.parts)s.type==="Literal"?t+=s.value:s.type==="DoubleQuoted"?t+=`"${s.parts.map(n=>Qr(n)).join("")}"`:s.type==="SingleQuoted"?t+=`'${s.value}'`:t+=Qr(s);return t}function Qr(e){let t=e;return t.type==="Literal"?t.value??"":t.type==="Variable"?`$${t.name}`:""}async function ei(e,t,s,n){let r="",i="",a=0;for(let o of t){if(!o){a=1;continue}let l=e.state.env.get(`BASH_ALIAS_${o}`);if(l!==void 0)n?r+=`${o} is an alias for "${l}"
`:r+=`alias ${o}='${l}'
...
`,127);if("script"in c)return t.includes("/")||(i.state.hashTable||(i.state.hashTable=new Map),i.state.hashTable.set(t,c.path)),await o(c.path,s,n);let{cmd:u,path:f}=c;t.includes("/")||(i.state.hashTable||(i.state.hashTable=new Map),i.state.hashTable.set(t,f));let d=n||i.state.groupStdin||"",h=a(),y={fs:i.fs,cwd:i.state.cwd,env:i.state.env,exportedEnv:h,stdin:d,limits:i.limits,exec:i.execFn,fetch:i.fetch,getRegisteredCommands:()=>Array.from(i.commands.keys()),sleep:i.sleep,trace:i.trace,fileDescriptors:i.state.fileDescriptors,xpgEcho:i.state.shoptOptions.xpg_echo,coverage:i.coverage,signal:i.state.signal,requireDefenseContext:i.requireDefenseContext,jsBootstrapCode:i.jsBootstrapCode,invokeTool:i.invokeTool},p=Kr(y,t);try{let w=()=>In(i.requireDefenseContext,"command",`${t} execution`,()=>u.execute(s,p));return u.trusted?await ve.runTrustedAsync(()=>w()):await w()}catch(w){if(w instanceof Y||w instanceof Qe)throw w;return _(`${t}: ${he(De(w))}
...
`,1)}let c=e.state.groupStdin;l&&(e.state.groupStdin=l);try{for(let f of t.body){let d=await n(f);r+=d.stdout,i+=d.stderr,a=d.exitCode}}catch(f){if(e.state.groupStdin=c,f instanceof Y)throw f;if(xt(f)||f instanceof pe||f instanceof B)throw f.prependOutput(r,i),f;return P(r,`${i}${De(f)}
`,1)}e.state.groupStdin=c;let u=P(r,i,a);return q(e,u,t.redirections)}async function $i(e,t,s,n,r){let i;try{i=await e.fs.readFile(t)}catch{return _(`bash: ${t}: No such file or directory
...
`,exitCode:2,env:me(this.state.env,s?.env)});if(f instanceof Ln)return this.logResult({stdout:"",stderr:`bash: ${he(f.message)}
