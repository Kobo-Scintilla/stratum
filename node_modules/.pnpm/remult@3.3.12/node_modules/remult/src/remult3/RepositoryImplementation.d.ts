...
import type { InsertOrUpdateOptions, QueryOptions, RefSubscriber, UpsertOptions } from './remult3.js';
import type { entityEventListener, packedRowInfo } from '../__EntityValueProvider.js';
...
export declare class RepositoryImplementation<entityType> implements Repository<entityType>, RepositoryInternal<entityType> {
    ...
    update(originalItem: Partial<MembersOnly<entityType>>, item: Partial<MembersOnly<entityType>>, options?: InsertOrUpdateOptions): Promise<entityType>;
...
export declare class rowHelperImplementation<T> extends rowHelperBase<T> implements EntityRef<T> {
    ...
    private edp;
    private _isNew;
    _updateResultsFromServerAction(rowInfo: packedRowInfo): Promise<void>;
    ...
    private _saving;
    save(options: InsertOrUpdateOptions | undefined, onlyTheseFieldsSentOnlyInTheCaseOfProxySaveWithPartialObject?: string[]): Promise<T>;
...
export declare const LabelTransformer: {
    /**
     ...
     */
    transformLabel: (remult: Remult, key: string, label: string, entityMetaData: EntityMetadata<any>) => string;
    /**
     * @deprecated use LabelTransformer.transformLabel instead
     */
    transformCaption: (remult: Remult, key: string, label: string, entityMetaData: EntityMetadata<any>) => string;
};
export declare function buildLabel(label: string | ((remult: Remult) => string) | undefined, key: string, remult: Remult, metaData: EntityMetadata<any>): string;
