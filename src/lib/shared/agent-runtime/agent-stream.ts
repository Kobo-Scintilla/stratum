...
import { persistToolResult } from './agent-context';
...
async function handleTextDelta(event: any, state: StreamState, stream: ActiveStream) {
	const t = event.delta;
	if (!t) return;
...
async function handleToolCallStart(event: any, state: StreamState, stream: ActiveStream) {
...
async function handleToolCallEnd(event: any, state: StreamState, stream: ActiveStream) {
...
function handleDone(event: any, state: StreamState, context: Context) {
	const content: (
